# Online_Shopping_System
<h2 style="font-size:18px;"> <b>LaSephora</b>is a feature-rich online shopping system built with HTML, Java, and MySQL. 
 <br><br>
 
 It streamlines cloth ordering, sales, and inventory management while providing robust tools for admin and supplier operations, user account management, and system maintenance.</h2>
 
<h2 style="font-size:22px;"><b>Features</b></h2>

<ul>
  
 <li> <b>Clothing Orders:</b> Simplifies the process of browsing and purchasing clothing items.</li>
  
  <li> <b>Data Management:</b>Efficiently handles user, order, and sales data for accuracy and easy retrieval.</li>
  
  <li><b>Sales Management: </b> Provides tools for tracking sales and generating insights.</li>
  
 <li><b>Secure User Authentication:</b> Features secure login, registration, and password reset functionalities to safeguard user data.</li>
  
 <li><b>Inventory Management: </b>Enables real-time stock tracking for admins and suppliers.</li>
  
 <li><b>Supply Management:</b> Optimizes supplier collaboration and order fulfillment.</li>

  
</ul>

<h2 style="font-size:22px;"><b>Database Management</b></h2>

The application uses MySQL within the MySQL Workbench server for efficient data handling and management, ensuring a reliable foundation for organizing and maintaining data effectively.

<b>Tables:</b>

<ul>

<li><b>Customer: </b>Stores customer details such as name, contact information, and account credentials.</li>
<li><b>Supplier: </b>Manages supplier information, including company details and product supply data.</li>
<li><b>Administrator: </b>Stores admin-related data.</li>
<li><b>newproduct: </b>Tracks information about newly added products, including descriptions, prices, and availability.</li>
<li><b>Orders: </b>Records customer orders, including product details, quantities, and order statuses.</li>
<li><b>Customer_Reviews:</b>Stores customer feedback and ratings for products and services.</li>

</ul>



<b>Triggers:</b> Auto-generation of primary keys for all tables ensures data integrity and consistency.


<h2 style="font-size:22px;"><b>Technologies Used</b></h2>
<ul>


  <li>
  <b>HTML, Java Servlets, JSP: </b>Primary programming language for the application.
    
  </li>
  <li>
    <b>CSS:</b> used for styling
  </li>
  <li>
  <b> MYSQL Workbench’s MYSQL Server:</b> Database management system.
  </li>
  <li>
   <b> Javascript:</b> For dynamic functionalities.
  </li>
</ul>


<h2 style="font-size:18px;"><b>A huge shoutout to my amazing teammates:</b></h2>
<b>Sanjana</b><br>
<b>Amath Perera</b><br>
<b>Thudakshana</b><br>
<br><hr>

<img width="1470" alt="Screenshot 2024-12-06 at 15 32 45" src="https://github.com/user-attachments/assets/292b5d62-9794-46f6-a339-683ef7270c2e">
<img width="1470" alt="Screenshot 2024-12-06 at 15 33 43" src="https://github.com/user-attachments/assets/5e6789ec-e4f7-45db-8092-11bf1552787a">
<img width="1470" alt="Screenshot 2024-12-06 at 15 34 00" src="https://github.com/user-attachments/assets/7e0e9e0d-cba3-4783-8ed6-d443aff27d4d">
<img width="1470" alt="Screenshot 2024-12-06 at 15 34 24" src="https://github.com/user-attachments/assets/f7b12e35-5fc2-4bcc-8c8b-ba491d2f1ce7">
<img width="1470" alt="Screenshot 2024-12-06 at 15 34 44" src="https://github.com/user-attachments/assets/e1e48226-a10a-4194-aed0-b3579542a5ad">
<img width="1470" alt="Screenshot 2024-12-06 at 15 35 32" src="https://github.com/user-attachments/assets/8d533ede-8490-4c6b-b8e2-744a05ab2940">
<img width="1470" alt="Screenshot 2024-12-06 at 15 35 43" src="https://github.com/user-attachments/assets/64bbb54d-16ab-44f4-81d3-99a7a668a23e">
<img width="1470" alt="Screenshot 2024-12-06 at 15 36 06" src="https://github.com/user-attachments/assets/145e6bb9-83a5-4581-b458-4cd2c63aab84">
<img width="1470" alt="Screenshot 2024-12-06 at 15 36 06 1" src="https://github.com/user-attachments/assets/c1e72231-1b5b-452f-8db9-dfc228a6a55b">
<img width="1470" alt="Screenshot 2024-12-06 at 15 36 45" src="https://github.com/user-attachments/assets/66e2444b-ddc0-40c6-938f-34d4da0ab5ef">
<img width="1470" alt="Screenshot 2024-12-06 at 15 37 43" src="https://github.com/user-attachments/assets/d6fc27dc-98cd-47b3-bfc2-4102c90c5d7d">
<img width="1470" alt="Screenshot 2024-12-06 at 15 38 12" src="https://github.com/user-attachments/assets/0656d40a-73cb-4d04-9b38-73e53371a7ab">
<img width="1470" alt="Screenshot 2024-12-06 at 15 38 30" src="https://github.com/user-attachments/assets/1bc96ef8-5e05-4314-814c-fd60002fd142">
<img width="1470" alt="Screenshot 2024-12-06 at 15 38 46" src="https://github.com/user-attachments/assets/5d65b434-2eb9-4252-9661-0a8e23df3629">
<img width="1470" alt="Screenshot 2024-12-06 at 15 39 37" src="https://github.com/user-attachments/assets/a1613a9f-804a-45b2-a745-89eaed559233">

<br><br><hr>


